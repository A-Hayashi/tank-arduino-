...
unsigned short I2C_Class::CRC16_Make(unsigned char *buf, int len) //CRC16-CCITT Format
{
...
boolean I2C_Class::CRC16_Check(unsigned char data[], int len, unsigned char crc[])
{
...
void I2C_Class::begin(long baud)
{
	Wire.begin(8);
	Wire.onReceive(this->Receive);
	
	L298N motor2(ENB, IN3, IN4);
}

/*
void I2C_Class::Send_Command(int sendCommand, int sendOption)
{	
	byte _packet[9];
	byte _crc[2];
...
void I2C_Class::Send_Processing(byte _data[], byte _length, byte _crc[])
{		
	byte _packet[30];
	...
	Wire.write(_packet, _length + 6);
}
*/
void I2C_Class::Receive()
{
	if (Wire.available() > 0)
	{	
		int input = Wire.read();
